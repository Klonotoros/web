...
function displayInfo(){
    ...
    if (blue_clicked)
        document.querySelector("#info1").textContent= "Nacisnąłeś fioletowy o wartości 1"
    if (red_clicked)
...
function yellowClick(){
    if (points>50 || document.getElementById("small-box").classList.contains("disabled"))
        return;
...
function redClick(){
    if (points>30 || document.getElementById("mid-box").classList.contains("disabled"))
        return;
    red_clicked = true;
    if (!propagation){
        displayInfo();
...
function blueClick(){
    if (document.getElementById("big-box").classList.contains("disabled"))
        return;
...
function clickHandler(id){
    if(!clicked){
        ...
        if(points>30){
            document.getElementById("mid-box").classList.add("disabled");
        }
        if(points>50){
            document.getElementById("small-box").classList.add("disabled");
        }
...
function reset(){
    ...
    changePoints(points);
    clicked = false;
    document.getElementById("mid-box").classList.remove("disabled");
    document.getElementById("small-box").classList.remove("disabled");
    clearInfo();
    resetbuttons();
...
document.getElementById("small-box").onclick = function() {
    clickHandler(this.id);
}

document.getElementById("mid-box").onclick = function() {
    clickHandler(this.id);
}

document.getElementById("big-box").onclick = function() {
    console.log("2");
    clickHandler(this.id);
    clickHandlerReset();   
}
